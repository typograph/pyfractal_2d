    ...
    def set_grid_dim(self, W, H):
        for col in range(self.gW, W):
            for row in range(self.gH):
                self.grid.layout().addWidget(ColorSelect(Qt.white, self.grid),
                                             row, col)
...
class MainWin(QtGui.QFrame):
    def __init__(self):
        ...
        self.imgLabel.setSizePolicy(QtGui.QSizePolicy.Expanding,
                                    QtGui.QSizePolicy.Expanding)
    ...
    def addColor(self, color):
        ...
        self.colors.layout().insertWidget(self.colors.layout().count() - 2, 
                                          FractalColor(color, self.colors))
